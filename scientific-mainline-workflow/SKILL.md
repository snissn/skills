...
description: Execute and govern theorem, model-integration, physics, manuscript-claim, or other scientific repository work directly on main without PR churn when that workflow is authorized. Use for mutable pre-freeze construction, independent adversarial review, local exact or quantitative validation, frozen scientific executions, scoped dispositions and inference, GitHub issue-graph progression, and strict separation of execution-only engineering defects from scientific results.
---
...
- broad code-review churn when the governing artifact is scientific rather than
  software-product code;
- replacing theorem review with CI volume or PR ceremony;
- treating an implementation defect as scientific evidence;
- creating governance packets that only restate already frozen scope;
- deepening one narrow certificate indefinitely when the active program needs
  breadth-first integration;
- importing deprecated or analogous work as authority instead of idea input.
...
- repository and current `main` identity;
- applicable `AGENTS.md` or equivalent instructions;
- current scientific goal and dependency graph;
- frozen inputs, source hashes, and protected claim boundaries;
- active issue and its pass, fail, stop, and inference rules;
- whether the work is analytic only or includes a decisive execution;
- user authority for direct commits and pushes to `main`;
- any live execution whose source or artifacts must remain untouched.
- the exact changed-path set, candidate hashes, reviewer ownership, and local
  validation commands that will constitute acceptance evidence.
...
exact base commit, changed paths, candidate artifacts, frozen inputs, and
acceptance predicates. Record the review disposition durably in a maintained
review artifact or the governing issue. A review of an earlier draft does not
authorize a later scientific edit.

Every delegated task must state:

- one precise scientific question;
- frozen inputs and relevant commit identities;
- owned files or responsibility;
- the required deliverable;
- accept, block, or scoped-failure criteria;
- non-goals and forbidden changes;
- the evidence standard;
- the stop condition;
- that recursive delegation is forbidden unless separately authorized.

Keep implementation and independent review separate. Treat subagent reports as
evidence to integrate, not as decisions to concatenate.
...
Resolve every blocking finding in the mutable draft. Repeat review until the
reviewer returns an explicit `ACCEPT` at the declared scope.
...
- Do not wait for a code-centric automated review when independent scientific
  review and the declared local gates already cover the artifact.
- Do not split ordinary direct-main scientific work into PR-ready and
  merge-ready phases unless the repository actually requires that workflow.
- Do not create authorization packets for obvious implementation.
- Do not create architecture or governance commits for ordinary runner
...
- Do not deepen one narrow certificate indefinitely when the program requires
  breadth-first integration.
- Do not import deprecated or analogous work as authority; use it as idea
  input and re-prove what is selected.
- Do not make downstream failures retroactively weaken frozen upstream
  theorems.
...
1. scientific disposition and exact scope;
2. files and commit changed;
3. independent-review result;
4. validation commands and outcomes;
5. GitHub issue and graph updates;
6. surviving results, blockers, and next selected dependency;
7. whether any scientific execution occurred.

Use the templates in [report templates](references/report-templates.md).

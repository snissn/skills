...
Keep unchanged heartbeats brief and answer explicit status requests concisely.
Preserve assigned readiness/merge authority until an explicit transfer; do not
wait for redundant approval or grant yourself permission to merge. Return the current head, complete

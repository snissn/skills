...
evidence, stop conditions, time box, and handoff. Every child must use exactly
`openrouter/stealth/ox-alpha`; a missing or mismatched actual route invalidates
its handoff. A worker may not merge, request external AI review, launch another
Pi process, or fall back to another model/provider.
...
Required route: openrouter/stealth/ox-alpha with thinking <LEVEL>.
...
- observed provider/model, which must be `openrouter/stealth/ox-alpha`;
...
RUN_DIR="$(mktemp -d /tmp/pi-ox-alpha-graph-worker-XXXXXX)"

...
You must run only as `openrouter/stealth/ox-alpha`. Never continue under, request,
or fall back to another provider or model; report any route mismatch immediately.
...
Hard rules:
- Verify the active route is `openrouter/stealth/ox-alpha` before substantive
  work and include the observed provider/model in the handoff.

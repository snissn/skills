#!/usr/bin/env python3
"""Launch and supervise one isolated Pi issue-graph worker.

The worker runs as a detached `pi --mode json --print --no-session` process.
Its JSON event stream and stderr remain in a run directory for coordinator
reconciliation. This helper intentionally does not interpret Git or GitHub
state; the parent Pi coordinator remains responsible for both.
"""
...
def worker_status(run_dir: Path) -> dict[str, Any]:
    ...
    if events.get("agent_end") and (stop_reason in TERMINAL_STOP_REASONS or events.get("error_message")):
        state = "failed"
...
def start_worker(args: argparse.Namespace) -> int:
    ...
    requested_provider, requested_model = split_model(args.model)
    meta = {
        "schema_version": 1,
        "started_at": utc_now(),
        "pid": process.pid,
...
def stop_worker(args: argparse.Namespace) -> int:
    ...
    status = worker_status(run_dir)
    if status["state"] != "running":
        print(json.dumps({"state": status["state"], "stopped": False, "reason": "worker is not running"}, indent=2))
        return 0
...
def build_parser() -> argparse.ArgumentParser:
    ...
    start.add_argument("--model", default=default_model(), help="Pi model pattern or provider/model")
    start.add_argument(
        "--thinking",
